...
function show_additional_information_meta_box() {
    ...
    $rating = get_post_meta( $post->ID, TR_GRABBER_FIELD_RATING, true ) == '' ? '' : get_post_meta( $post->ID, TR_GRABBER_FIELD_RATING, true );
    ...
    echo '<table class="tr_grabber_content form-table">
            <tbody>
                ...
                <tr>
                    <th>
                        <label><span class="dashicons dashicons-clock"></span>'.__('Rating', 'tr-grabber').'</label>
                    </th>
                    <td>
                        <input type="text" name="rating" value="'.$rating.'" placeholder="'.__('Rating', 'tr-grabber').'">
                    </td>
                </tr>
